...
type CreateUpdate = "Create" | "Update" | "Top";
...
class Visitor extends ReactiveFunctionVisitor<CreateUpdate> {
  ...
  join(values: Array<CreateUpdate>): CreateUpdate {
    ...
    return values.reduce(join2, "Top");
  }
  ...
  override visitPlace(
    ...
    this.map.set(
      place.identifier.id,
      this.join([state, this.map.get(place.identifier.id) ?? "Top"])
    );
  }
  ...
  override visitInstruction(instruction: ReactiveInstruction): void {
    const state = this.join(
      [...eachInstructionLValue(instruction)].map(
        (operand) => this.map.get(operand.identifier.id) ?? "Top"
      )
    );
...
export default function pruntInitializationDependencies(
  fn: ReactiveFunction
): void {
  const [aliases, paths] = getAliases(fn);
  visitReactiveFunction(fn, new Visitor(aliases, paths), "Update");
}
